...
const styleBack    = './src/sass/style.sass';
const styleFront   = './src/sass/front.sass';
const styleURL     = './css/';
const mapURL       = './';


const jsSRC        = './src/js/';
const jsBack    	 = 'scripts';
const jsFront      = 'front';
const jsURL        = './js/';
const jsAll				 = [ jsBack, jsFront ];
...
gulp.task('styles', function() {
	return gulp.src([ styleBack, styleFront ])
		.pipe(sass({ outputStyle: 'expanded' })) // expanded or compressed
		//.pipe(concat('styles.min.css')) // заремарив бо включив обробку двох файлів
		.pipe(autoprefixer({
			grid: true,
